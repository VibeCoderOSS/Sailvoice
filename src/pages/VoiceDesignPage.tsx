...
import { createDesignVoice, deleteVoice, listVoices, renameVoice, requestVoicePreview } from '../api/client';
...
export function VoiceDesignPage() {
  ...
  const designVoices = useMemo(() => voices.filter((voice) => voice.type === 'design'), [voices]);
  ...
  return (
    <section className="page">
      <h2 className="page-title">{t('navVoiceDesign')}</h2>

      <div className="grid-two">
        <article className="panel">
          <h3>{t('designVoiceSection')}</h3>
            ...
            <article key={voice.id} className="voice-item">
              <div className="row-between">
                <div>
                  ...
                  <p className="kv">
                    {t('voiceType')}:{' '}
                    <span className="pill pill-design">
                      {t('designType')}
                    </span>
                  </p>
                  <p className="kv">{voice.language || t('auto')}</p>
                  {voice.description ? <p className="kv">{voice.description}</p> : null}
                </div>
